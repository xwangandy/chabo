...
class UpdateHandler:
    ...
    def _send_advertiser_menu(self, chat_id: str | int, user: dict[str, Any], source_message: dict[str, Any] | None = None) -> None:
        ...
        self._reply_or_edit(
            ...
            inline_keyboard=[
                [{"text": "🔍 找频道", "callback_data": "advertiser:discover"}, {"text": "➕ 广告投放", "callback_data": "advertiser:order_help"}],
    ...
    def _send_advertiser_library(self, chat_id: str | int, user: dict[str, Any], source_message: dict[str, Any] | None = None) -> None:
        ...
        if creatives:
            for index, creative in enumerate(creatives, start=1):
                ...
                lines.append(
                    f"   {self._creative_status_label(creative['status'])} · "
                    f"{self._slot_name(creative['format_type'])} · {short} · {creative['button_text']}"
                )
        else:
            lines.extend(
                [
