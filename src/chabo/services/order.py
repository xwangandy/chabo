...
class MaterialService:
    ...
    def create_material(
        ...
        *,
        ...
        audit_args = {
            ...
            "has_standard_text": bool(standard_text),
        ...
        *,
        ...
        with self.db.transaction() as conn:
            ...
            sql = (
                ...
                "WHERE (cr.advertiser_account_id = ? OR (cr.advertiser_account_id IS NULL AND ca.advertiser_account_id = ?))"
            )
            params: list[Any] = [advertiser["id"], advertiser["id"]]
    ...
    def list_materials_in_conn(
        ...
        *,
        ...
        params: list[Any] = [advertiser_account_id, advertiser_account_id]
        if format_type is not None:
            sql += " AND cr.format_type = ?"
            params.append(self._normalize_format(format_type))
        if not include_archived:
    ...
    def _insert_material(
        ...
        *,
        ...
        content_hash = hashlib.sha256(
            f"{format_type}|{light_short_text or ''}|{standard_text or ''}|{media_file_id or ''}|{text}|{target_url}|{button_text}".encode("utf-8")
        ).hexdigest()
        conn.execute(
            """
            INSERT INTO creatives (
                id, campaign_id, advertiser_account_id, format_type,
                text, target_url, button_text, category, short_text,
                standard_text, media_file_id, media_type, light_short_text,
                status, content_hash
            )
        ...
        with self.db.transaction() as conn:
            ...
            if material_id is not None:
                material = conn.execute(
                    """
                    SELECT cr.*, ca.advertiser_account_id AS campaign_advertiser_account_id
                    FROM creatives cr
                    JOIN campaigns ca ON ca.id = cr.campaign_id
                    WHERE cr.id = ?
                    """,
                ...
                if not material:
                    raise NotFound(f"广告素材不存在：{material_id}")
                material_owner_id = material["advertiser_account_id"] or material["campaign_advertiser_account_id"]
                if material_owner_id != advertiser["id"]:
                    raise NotFound(f"广告素材不存在：{material_id}")
                if material["advertiser_account_id"] is None:
                    conn.execute(
                        "UPDATE creatives SET advertiser_account_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        (advertiser["id"], material_id),
                    )
                ...
                creative_snapshot = {
                    ...
                    "standard_text": material["standard_text"],
        ...
        *,
        ...
        with self.db.transaction() as conn:
            ...
            creative = conn.execute(
                ...
                "short_text, standard_text, media_file_id, media_type "
